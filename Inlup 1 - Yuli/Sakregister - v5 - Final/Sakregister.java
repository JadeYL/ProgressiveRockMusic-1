...
//		}
		...
		btnVisa.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//Makes TextArea AppendSafe from setting text.
				if(textArea != null){
					textArea.setText("");
				}
				
				//3 Threads split at once. Unessesary but helps.
				Thread t1 = new Thread(){
					public void run(){
						try {
							gate.await();
							textArea.append(allaSmycke.toString().replace("[", "").replace("]", "").replace(", ", ""));
				...
				Thread t2 = new Thread(){
					public void run(){
						try {
							gate.await();
							textArea.append(allaAktie.toString().replace("[", "").replace("]", "").replace(", ", ""));
						} catch (InterruptedException | BrokenBarrierException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
				...
				Thread t3 = new Thread(){
					public void run(){
						try {
							gate.await();
							textArea.append(allaApparat.toString().replace("[", "").replace("]", "").replace(", ", ""));
						} catch (InterruptedException | BrokenBarrierException e) {
							// TODO Auto-generated catch block
							e.printStackTrace();
						}
...
//					String fab = textArea.getText();
//					
//					
//					String sortValue = fab.replaceAll("värde:", "#");
...
//						indexListFirst.add(firstindexofSortVal);															
//						
//						//Arrays.sort(sortValues, 10, 15); //sortValues[iix].len
//						
//					}		
					
					/** Obsolete code v **/
					String sortVal = textArea.getText();
					...
					//SORTING DONE
					allaSmycke.sort((y, x) -> Integer.compare((int)x.getValue(), (int)y.getValue()));
					allaAktie.sort((y, x) -> Integer.compare((int)x.getValue(), (int)y.getValue()));
					allaApparat.sort((y, x) -> Integer.compare((int)x.getValue(), (int)y.getValue()));
					
					
					/** FULL OBSOLETE CODE BENEATH **/
					
/**		this one is for sorting names:			Collections.sort(superList,(o1,o2)-> o1.getClass().getName().compareTo(o2.getClass().getName()));  //all this does is reset to original **/

					
					ArrayList<Integer> indexListFirst = new ArrayList<Integer>();
					ArrayList<Integer> indexListLast = new ArrayList<Integer>();
					
					textArea.setText(superList.toString().replace("[", "").replace("]", "").replace(", ", ""));
					
					/** Counts to first index **/
					for(int iix=0;iix<sortValues.length;iix++){  //plus one since the actual array somehow wants to start at 1.
						System.out.print("Found index at: ");
						System.out.println(sortValues[iix].indexOf("värde:"));	 //then use index+1 cause its after that											
						int firstindexofSortVal = sortValues[iix].indexOf("värde:"); 
...
//					for(int iip=0;iip<sortValues.length;iip++){
//						int lastindexofSortVal = sortValues[iip].length();
//						indexListLast.add(lastindexofSortVal);
//					}
//					
					for(int iiy=0;iiy<indexListFirst.size();iiy++){
						System.out.println(indexListFirst.get(iiy));
						System.out.println(sortValue2.length());
						//Arrays.sort(sortValues, indexListFirst.get(iiy), indexListLast.get(iiy));  //doest even try to cooperate.
					}
...
//							NyApparat1 nyap = new NyApparat1();		
//							Apparat app = new Apparat(getApparatNamn(), getInkopsPris(), getSlitage());  //added cast int later when I switch everything to float.
//							/**  perhaps have helper class here **/
							allaApparat.add(appa);
